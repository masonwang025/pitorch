/*
 * 4-Pi pipeline training for stories110M over GPIO ring.
 * Each Pi loads a weight SHARD (not the full model).
 *
 * Topology (110M = 12 layers, shared weights → embed+head on same rank):
 ...
 *
 * Memory per rank (gpu_mem=32, 480 MB ARM RAM):
 *   R0/R1/R2: shard=108MB + grads=108MB + wt=6MB ≈ 233MB (49%)
 *   R3:       shard=94MB  + grads=94MB  + wt=94MB ≈ 295MB (61%)
 *
...
void notmain(void) {
    ...
    pt_dist_print(&dist);
    printk("110M training: rank %d, n_local=%d, dim=%d, act_bytes=%d\n",
           RANK, n_local, dim, act_bytes);
    ...
    printk("=== train_4pi_110M rank %d DONE ===\n", RANK);
    clean_reboot();
}
